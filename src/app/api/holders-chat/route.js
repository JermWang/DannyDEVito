...
export async function GET(req) {
  ...
  try {
    const messages = await prisma.holdersChat.findMany({
    ...
    const sorted = messages.reverse().map((m) => ({
      id: m.id,
      wallet: m.wallet,
      walletShort: m.wallet.slice(0, 4) + "..." + m.wallet.slice(-4),
    ...
    console.error("Failed to fetch holders chat:", error);
...
export async function POST(req) {
    ...
    if (ban) {
      if (!ban.expiresAt || new Date(ban.expiresAt) > new Date()) {
        ...
        return NextResponse.json(
          { ok: false, error: "banned", reason: ban.reason || "Spam abuse", expiresIn: remaining },
          { status: 403 }
        );
      } else {
  ...
  if (timeSinceLast < MESSAGE_COOLDOWN_MS) {
    ...
    return NextResponse.json({ ok: false, error: "cooldown", waitSeconds: waitSec }, { status: 429 });
  }
  ...
  const nickname = typeof body?.nickname === "string" ? body.nickname.trim().slice(0, 32) : null;
  ...
  if (recentCount >= SPAM_THRESHOLD || (isRepeat && recentCount >= 3)) {
    ...
    return NextResponse.json(
      { ok: false, error: "banned", reason: "You've been temporarily banned for spamming", expiresIn: Math.ceil(BAN_DURATION_MS / 1000) },
      { status: 403 }
    );
  }

  try {
    const entry = await prisma.holdersChat.create({
      data: { wallet, nickname, message, color },
    });
    ...
    console.error("Failed to save holders chat message:", error);
    return NextResponse.json({ ok: false, error: "database_error" }, { status: 500 });
  }
}
